...
/**
 * P4-3 acceptance: 击破伤害公式。
 *
 * <p>锚点（ROADMAP P4-3）：攻击者 Lv80、击破特攻 300%、削韧 112.5 点、敌防 1150、无抗性无减伤 →
...
public class BreakDamageTest {
    ...
    public void breakingRateIsLoadedFromData() {
        Assertions.assertEquals(3767.5535, Constant.BREAKING_RATE.get(80), 1e-4, "数据文件是 10 倍值");
    ...
    public void breakDamageIsBaseTimesBreakingEffectTimesStanceThroughDefence() {
        ...
        double expected = expectedBase * (1000.0 / (DEFENCE + 1000.0));        // 防御区（Lv80 → 1000）
    ...
    public void breakDamageIgnoresBoostAndNeverCrits() {
        Character breaker = breaker(3.0);
        breaker.setAttribute(AttributeType.ALL_DAMAGE_TYPE_BOOST, new DoubleValue(5.0));   // 增伤 500%
        Enemy dummy = dummy();
        Battle battle = new Battle(List.of(breaker), List.of(dummy), new Random(0));
        ...
        Assertions.assertEquals(78_855.8, settled, 1.0, "击破伤害不吃增伤");
