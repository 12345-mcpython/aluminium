...
**当前状态（2026-09-27）**：**90 个测试类 / 818 个用例全绿**；demo 跑通一场三对三战斗
...
| F-10 | 套装具名 ability 只有一部分能表达 | ✅ **账本已完整**（2026-09-27，见 `M-25`）：带 ability 的效果共 **64 条** —— `35 纯 ability` + `29 数值+ability 混合`；其中**已表达 17 条**（10 纯 + 7 混合）、**已登记 47 条**（25 + 22），由 `RelicTriggerTableTest` 钉住 `35 + 29 = 17 + 47`（两个分母各自钉住，所以效果不能靠长出一个 `properties` 逃出分区）。登记表里 **7 条**标明 `Writable now:`（能力已有、只差写文件）并被计数钉住。**剩下的是内容活，不是能力活** |
