...
**当前状态（2026-09-27）**：**88 个测试类 / 794 个用例全绿**；demo 跑通一场三对三战斗
...
- **遗器套装效果**：✅ **已落地**（`relic_sets.json` 装载 + 2/4 件套生效 + `RelicFactory` 确定性遗器
  ...
  > **35 条里精确表达了 10 条、登记了 25 条**（`_unmodelled.json` + `F-10`，各自写明缺哪种能力）。
...
| M-25 | `RelicTriggerTableTest.everyAbilityOnlyBonusIsEitherAuthoredOrRegistered` | **登记表只覆盖了一半**（2026-09-27 普查发现）：那条测试的分区（`35 = 已写 + 已登记`）开头就 `if (!effect.properties().isEmpty()) continue;`，所以只统计 `properties` **为空**的纯 ability。数据里另有 **29 条**效果是「无条件数值 + 具名 ability」的**混合**形状（其中 24 条是平面饰品 2 件套），它们的第一句由 `properties` 正常生效、**第二句（带条件的那半）被静默丢掉且不在登记表里** —— 于是"没写规则文件"和"忘了写"在这一类上完全一样，正是该测试声称要守住的区别。**危害**：一批套装效果少生效一部分，只是数值偏小，没有任何报错。<br>**修法**：把混合效果也纳入分区校验（分母 35 → 64），未表达的逐条登记原因。⚠ 注意 2026-09-27 用 `self_attr` 写掉的 301/307/309 **没有**改变任何计数 —— 这恰好证明缺口是真实的，不是记账问题。<br>**顺带**：`theRegistryCoversTheTwoPiecesTierToo` 的注释说"六个 2 件套"也过时了（当前登记表 5 条，而数据里 2 件套能力的真实数量远大于此） |
...
| F-10 | 套装具名 ability 只有一部分能表达 | 92 条效果里 35 条是纯 ability：**精确表达了 10 条、登记了 25 条**（`_unmodelled.json`，每条写明缺哪种能力），由 `RelicTriggerTableTest` 钉住 35 = 10 + 25。⚠ **这个分母漏了 29 条"数值 + 具名 ability"混合效果**（见 `M-25`）：它们带条件的那半此前静默失效且无人登记；2026-09-27 用 `self_attr` 写掉 3 条（301/307/309），这类效果**既不在分母里、也还没有登记表** |
