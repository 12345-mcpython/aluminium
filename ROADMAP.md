...
**当前状态（2026-09-27）**：**93 个测试类 / 852 个用例全绿**；demo 跑通一场三对三战斗
...
| **忆灵面板（继承）** | **忆灵的面板 = 召唤者的份额 + 平项**（2026-09-27）：`resources/memosprites/<召唤者 cid>.json` 写"怎么推导"而不是数值 —— `值 = percent × 召唤者该属性 + flat`，读的是召唤者**已解析**的面板，所以光锥/遗器/行迹/buff 自动跟着走。同一形状覆盖"35% 速度"（只有 percent）、"160 点速度"（只有 flat）、"66% + 720"（两者都有）。⚠ 一条 entry 是**替换**不是叠加 → `HEALTH`/`SPEED` 必填、装载期拒绝并说明后果（0 血上限会被一次 tick 打死、0 速排不进行动条）。首个用户 **1402 阿格莱雅·衣匠**（35% 速度 / 66% 生命上限 + 720）与 **1413 长夜月·「长夜」**（160 点速度 / 50% 生命上限），见 `engine.md` §24.5。**仍缺**：忆灵技与行动、ATTACK/DEFENCE 无数据、按属性缩放的伤害、重复召唤的"刷新" |
| **召唤忆灵（`SUMMON`）** | **规则能把忆灵召上场**（2026-09-27）：无参数的 `SUMMON`，作用于规则主人 —— **1413 长夜月**「进入战斗时召唤忆灵「长夜」」= `BATTLE_START → SUMMON`、**1402 阿格莱雅**终结技「召唤忆灵衣匠」= `ULT_CAST` + `actor == self` → `SUMMON`。不收 `target`（多写**报错**而不是忽略：忆灵属于召唤者，没有可指的对象）；**按召唤者幂等**（第二次保留已在场那个 —— 两个同样的忆灵是玩家看不见的错误状态；文档的"若已在场则回满"刷新**未建模**、已登记）。⚠ **用了 `SUMMON` 却没有忆灵文件的角色在装配时就被拒**：检查必须在装配点，因为规则文件不知道自己属于谁、而遗器规则是每个穿戴者共享的。见 `engine.md` §24.7 |
| **「忆灵在场」问得出、也指得到** | **`self_summon_count` + `"target": "summon"`**（2026-09-27）：条件读的是**规则主人自己活着的召唤物数**（按阵营查，所以敌方 Boss 的小怪也算；数**全部**而不是第一个 —— 知更鸟·晴歌的「晴空乐手」是三个），目标选择器解析为**主人的召唤物**（找不到时**响亮报错并告诉你加那个条件**）。因为这是**战场**的事实而不是事件的事实，`TriggerContext` 现在携带 `Battle`；**手搓的 context 没有战场时这类条件一律不成立**，绝不退化成"0 个"（`== 0` 这种反向问法专门钉这条）。⚠ 顺带修掉一个文档谎言：`==`/`!=` 此前一律走身份分支，所以文档里写了很久的 `hit_count == 2` **根本解析不了**。首个用户 **318 奇想蕉乐园** 2 件套（有召唤物时暴击伤害额外 +**32%**，用 `TURN_START` + 1 回合重判，而不是 `permanent` —— 否则召唤物死了 buff 还在），见 `engine.md` §24.6 |
| **召唤物（两个阵营）** | **召唤物能真的上场了，而且我方也行**（2026-09-27，P9-4 + L-8 友方那一半）：`monster_config.summon_id`（**692/2649 只怪有非空名单**）→ `Enemy.summonIds` → `SummonFactory`（复用 `EnemyFactory.resolve` 的缩放链）→ `Battle.summon(master, summonId, group)`，**加入主人自己的阵营**；**主人倒下带走它**。⚠ 两处别记反：①退场**不付击杀奖励**，原因是这次清扫跑在 `Battle.applyDamage`（唯一结算入口）**之外** —— 不是"`takeDamage` 会发事件"（它一个都不发，这正是一条被变异测试纠正的错判，见 `engine.md` §6.2）；②`CanHit.perish()` 只负责"别谎报它受了伤"（**HP 不变**）。`SummonTest` 18 条 + `PlayerSideSummonTest` 12 条 —— 后者把**每个阵营级读取点**各钉一条，见 `engine.md` §24 |
| **无视防御（持久型）** | **「无视目标 X% 的防御力」= 数据，不需要新 op**（2026-09-27 核实）：防御区的"穿透"参数取自**攻击者的 `DEFENCE_IGNORE` 属性**，所以 `MODIFY_ATTR DEFENCE_IGNORE 0.12` 就够了。第一个用户是**翡翠 1314 星魂 4**（「施放终结技时…无视 12% 防御力，持续 3 回合」）—— 这也是属性存在以来**第一次**被内容使用，`DefenceIgnoreTest` 9 条把"规则 → 属性 → 防御区"这条此前从未跑过的链路钉住。⚠ 两个容易踩的点：属性是**比率**（0.12 就是 12%）；默认 `max_stacks: 1` 意味着**两个来源是替换不是相加**（波提欧 16% + 阮·梅 20% = 20%），要相加得显式写 `max_stacks`。**仍缺**的是把无视**限定在某一击**的那些（银枝/云璃/遗器 119），见 `engine.md` §3.2 |
| **自身属性门槛** | **「当装备者的 X ≥ N 时」= 数据**（2026-09-27）：条件 DSL 新增 `self_attr:<属性>`，属性名走 `AttributeType`（封闭、已校验）—— 所以这是**加名字而不是加引擎**，正是"高可扩展性"那条原则要的形状。它是唯一**带参数**的数值变量，读的是**规则主人自己**的属性（读成 `actor` 会让"别人快"决定"我的"机制，且日志里看不出来）。⚠ 量纲：基础属性是绝对值（`self_attr:SPEED >= 145`），比率属性是分数（`self_attr:CRIT_CHANCE >= 0.7` 即 70%），写成 `>= 70` 能加载却**永不触发**；`*_PERCENT` 那四个装载期直接拒绝（运行时槽位是 null，读它会在伤害结算里 NPE）。覆盖面是普查里最大的：**29 条混合效果中 13 条**只差这一个条件。已写掉 **7 条位面饰品 2 件套**：**301 太空封印站 / 302 不老者的仙舟（我方全体，带 `target: all_allies`）/ 304 筑城者的贝洛伯格 / 306 停转的萨尔索图（终结技+追加两条作用域）/ 307 盗贼公国塔利亚 / 309 繁星竞技场（普攻+战技）**，外加 **308 生命的翁瓦克**用 `ADVANCE`（进战斗行动提前 40%），见 `engine.md` §4.6 |
...
- ⚠ **比对 demo 要比全文，不能只 `Select-String "Battle over"`**（2026-09-27 踩到）：
  那句只证明**胜负与轮数**没变。我这轮给遗器套装 301 加了规则文件，而 demo 队穿的**正是 301** ——
  过滤成一行就"看起来没变"，实际伤害数值可能已经变了。正确做法是取工作树里改动前的提交
  （`git worktree add <临时目录> <commit>` + 把 `data/` 内容拷进去）跑两遍全文再 `Compare-Object`。
  实测：281 行里只有 3 行 `Resist {...}` 的顺序不同，而那是 `Main` 打印 `Set` 的**既有**不稳定行为
  （同一份代码跑两次也会变，见 P10-0 的注），数值/HP/击杀逐行一致。
- **文件编码**：`Get-Content | Set-Content` 会把 UTF-8 文档按 GBK 往返一次，**整份糊成乱码**
  （2026-09-27 实测把 `ROADMAP.md` 写坏了一次）。中文文档一律走编辑器/fs 工具，不要用 shell 重写；
  真写坏了就 `git checkout -- <file>` 还原后逐条重打。
...
4. ~~**我方主人的召唤被响亮拒绝**~~ —— ✅ **当天就补上了**：`Battle.allies`（我方阵营，
   `List<CanHit>`）与 `characters`（我们的角色，子集视图）分开，`Battle.summon` 按**主人的阵营**
   决定进哪个名单（`Camp.NEUTRAL` 仍拒绝）。这是 L-8 的友方那一半，见 `engine.md` §24.4。
   当时拒绝的理由是"我方唯一的名单是 `List<Character>`、放不下 `Summon`"，而计划里把"我方召唤物入场"
   归为"内容活"是错的 —— 它是**类型问题**。

**还不做的**（都记在 `engine.md` §24.3）：
- **忆灵技与行动** —— 忆灵的技能表不在本数据里，所以它**没有技能**，暂时不会做出有意义的行为
  （面板 §24.5 / 在场 §24.6 / 召上场 §24.7 都已就位）。
- **按属性缩放的伤害** —— 文档写「等同于忆灵 X% **生命上限**」或「等同于**召唤者**攻击力 Y%」，
  与 §23.6 的"amount 是字面量"是同一个缺口；没有它，忆灵即使有技能也打不出文档的伤害。
- **重复召唤的"刷新"** —— 阿格莱雅的「若已在场，则使其生命值回复至上限」；现在是无操作（幂等）。
- **敌人技能侧的 `SkillEffectType.SUMMON` 分派** —— 数据里没有"这次施放召谁"那一列，缺的是**内容**
  不是能力（能力就是 `Battle.summon`，而**角色侧**已经能靠 `SUMMON` op 召了，见 §24.7）。
- **谁来触发敌方召唤** —— `summon_id` 名单只回答"能召谁"，不回答"什么时候召"；调用方（将来的敌方技能 /
  阶段表 / 敌方 AI）负责。这条与原计划一致。
...
- **遗器套装效果**：✅ **已落地**（`relic_sets.json` 装载 + 2/4 件套生效 + `RelicFactory` 确定性遗器
  ...
  > **35 条纯 ability 里精确表达了 10 条、登记了 25 条**（`_unmodelled.json` + `F-10`，各自写明缺哪种能力）。
  > ⚠ 这 35 条只算 `properties` **为空**的效果；另有 **29 条**「无条件数值 + 具名 ability」的混合效果
  > 原先从未进过分区。2026-09-27 已把两类都纳入（`M-25`）：**64 = 17 已写 + 47 已登记**。
...
| M-25 | `RelicTriggerTableTest` | ✅ **已修（2026-09-27）**：登记表原先只覆盖一半。那条分区测试（`35 = 已写 + 已登记`）开头就 `if (!effect.properties().isEmpty()) continue;`，所以只统计 `properties` **为空**的纯 ability。数据里另有 **29 条**效果是「无条件数值 + 具名 ability」的**混合**形状（24 条是平面饰品 2 件套），它们的第一句由 `properties` 正常生效、**第二句（带条件的那半）被静默丢掉且不在登记表里** —— 于是"没写规则文件"和"忘了写"在这一类上完全一样，正是该测试声称要守住的区别。<br>**修法**：把混合效果也纳入分区（**两个分母各自钉住**：`35 纯`、`29 混合`，不变式 `35 + 29 = 已写 + 已登记`），未表达的逐条登记原因。⚠ 同一个盲区在**第二个测试**里也有一份：`theRegistryNamesRealAbilities` 同样带 `properties().isEmpty()`，所以 22 条新登记的混合项一落地就全被判成"过时条目" —— 两处一起修，并且**故意的变异**（把过滤条件放回去）确认各红一条。<br>**顺带把账本填实**：29 条混合里 **7 条已写**（301/302/304/306/307/308/309，`self_attr` + `ADVANCE` 让它们第一次可表达），其余 **22 条**逐条写明缺哪种能力；其中 **7 条**能力已有、只差写文件（`reason` 以 `Writable now:` 开头且数量被钉住，`theWritableBacklogIsLabelledAndCounted`），免得"引擎补了口子、登记表还在说缺口子"。⚠ **写理由时还抓到我自己一个错判**：7 条登记把「两级阈值（≥135/160）」当成阻塞理由，其实**不是缺口** —— 高的一档带下界、低的一档带上界（`self_attr:SPEED >= 135` 且 `< 160`）就互斥了，`when` 本来就是与关系。7 条理由当天全部改正，写法记在 `engine.md` §4.6。<br>**意义**：这条缺陷的危害是"一批套装效果少生效一部分，数值偏小、零报错"；修完之后的账本是 `64 = 18 + 46`，`_unmodelled.json` 第一次成为**完整**的待办清单 |
| M-26 | `TriggerInterpreter` 的 op 校验 | **把 `target` 写在"本来就只作用于主人"的 op 上会被静默忽略**（2026-09-27 加 `SUMMON` 时发现）。`requireTargetSelector` 只校验**取值**是不是已知选择器，从不问"这个 op 收不收 `target`"；于是 `GAIN_ENERGY` / `GAIN_SKILL_POINT` / `BOOST_DAMAGE` / `REMIT`… 上的 `target` 会被 Gson 读进来、被校验通过、然后**没人用** —— 作者写的是"给某个队友回能"，引擎做的是"给主人回能"，两边都不报错。<br>**已核**：出货内容里**没有**一条这么写，所以这是潜在而非现实的漏洞。<br>**已修一半**：`SUMMON` 现在在加载期拒绝 `target`（`requireNoTarget`），因为它是新加的、没有历史包袱。<br>**未修**：其余六个"只作用于主人"的 op。一次收紧它们会**改变七个 op 被接受的词汇表**，属于单独一件事 —— 建议连同 `requireNoTarget` 一起加，并跑一遍全仓内容确认没有规则依赖旧的容忍行为 |
...
| F-10 | 套装具名 ability 只有一部分能表达 | ✅ **账本已完整**（2026-09-27，见 `M-25`）：带 ability 的效果共 **64 条** —— `35 纯 ability` + `29 数值+ability 混合`；其中**已表达 18 条**（10 纯 + 8 混合）、**已登记 46 条**（25 + 21），由 `RelicTriggerTableTest` 钉住 `35 + 29 = 18 + 46`（两个分母各自钉住，所以效果不能靠长出一个 `properties` 逃出分区）。登记表里 **10 条**标明 `Writable now:`（能力已有、只差写文件）并被计数钉住。**剩下的是内容活，不是能力活** |
