...
| L-8 | `Battle.java:257` | ✅ **一半已修（2026-09-26）**。原文：`Battle.enemies` 是 `List<Enemy>` 不是 `List<? extends CanHit>` → 敌方召唤物**无处安放**（P9-4 没写"要先放宽类型"）。<br>**已修**：`enemies` 改为 `List<CanHit>`（构造器收 `List<? extends CanHit>` 并**拷贝**，不再与调用方 list 别名）；新增 `Battle.enemyUnits()` = 其中的 `Enemy`；`targetableEnemies()` / `WaveManager.aliveEnemies()` 一并返回 `CanHit`；需要怪物语义的 12 个测试文件显式改用 `enemyUnits()`（**25 处**，全部是"这个测试真的在测怪"的地方）。**能力回归**：`EnemyCampSummonTest` 4 条；变异（胜负判定改 `enemyUnits()`、目标表限回 `Enemy`）各红一条。全套 66/617 绿，demo 除行动条外不变。<br>**未修的另一半**（原条目的其余部分）：`addRequest` 仍是 private 零调用、`addRequestItems` 是 public 字段、`advanceRequest` public 零调用、`Summon` 仍无内容侧创建（那是 P9-4 的内容活） |
...
| L-26 | `Queue.java:539`、`Signal.java:142` | ✅ **已修（2026-09-26）**。原文：`delayAction` 只加 `nextActionTime`，**不同步 `Signal.remaining`**；`refreshSpeed` 又按**陈旧的** `remaining` 算进度并**覆盖** `nextActionTime`，`clamp(progress, 0, 1)` 还会把"已推过一整个周期"的单位拉回恰好一个周期 → 已生效的推条被静默丢掉。**实测**：量子击破"先推条后减速"时，行动值变化**加不加那 20% 额外推条都是 28.409**。<br>**修法（两半都要）**：① `delayAction`/`advanceAction` 调 `setRemaining` 同步两个账本；② `refreshSpeed` 去掉**上**钳（`max(0, …)`），因为被推条的单位合法地离行动点超过一整轮。**回归**：`QueueActionManipulationTest.aDelaySurvivesASpeedChange`（先写红再修）；两半各自都能让它变红 —— 恢复上钳 → `a=b=187.5`；去掉账本同步 → `a=b=187.5`。全套 65/613 绿，demo 除行动条外逐行不变。见 `engine.md` §5.1/§5.2 |
