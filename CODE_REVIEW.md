...
| L-7 | `Battle.java:145` | 无敌目标的 DOT **照样消耗层数**却零伤害（`tick()` 无条件调用，`applyDamage` 对 `isInvulnerable()` 返回 0）→ 转阶段无敌的 Boss 白吃 DOT 结算次数。（同段"被 DOT 打死就 break"是对的，不鞭尸） |
...
| L-12 | `Dot.java:42` | 构造器不校验：`remainingTurns <= 0` 会"第一次结算就过期"，任意元素都收（`DOT_ELEMENTS` 过滤只在 `Battle.attachBreakDot`），负 `baseDamage` 静默不造成伤害 |
