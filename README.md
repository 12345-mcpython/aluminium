...
| `ROADMAP.md` | **开发路线图**：设计原则（引擎不认角色 / 三分法）+ 待办任务（每个自带验收标准）+ 踩坑记录 |
| `CODE_REVIEW.md` | 一次全量代码审查的问题清单（严重度分级 + 修法建议 + 修复进度） |
| `DOC_VS_CODE.md` | 文档与代码的不一致记录（含"文档说的与实现不符"的逐条证据 + §F 引擎能力缺口登记） |
| `docs/archive/ROADMAP-v2-历史.md` | ROADMAP v2 全文存档：已完成任务的完整规格与当时的实测数字 |

> ⚠ `DOC_VS_CODE.md` 里"`ROADMAP` 第 N 行"的行号指向 **v2**（存档文件），
> 因为 2026-09-23 的 v3 重写把 ROADMAP 从 2446 行压到约 350 行，行号已失效。
> 任务编号（`P3-4` 等）仍然有效。
