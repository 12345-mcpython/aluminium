...
| `docs/archive/ROADMAP-v2-历史.md` | ROADMAP v2 全文存档：已完成任务的完整规格与当时的实测数字（**纯历史**，不再维护） |

> 2026-09-26 删掉了 `CODE_REVIEW.md` 与 `DOC_VS_CODE.md`（一次性的审查/不一致分析快照，已多处腐烂）。
> 它们的**未修条目**全部并进了 `ROADMAP.md` §12；已修条目不保留（全文在删除前的提交里）。
> 旧文档里"`ROADMAP` 第 N 行指向 v2"的行号说法随之作废 —— 现在只用任务编号（`P3-4` 等）引用。
