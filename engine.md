...
1. **忆灵技与行动**：忆灵的技能表不在本数据里，所以它**没有技能**，暂时不会做出有意义的行为；
2. **在场条件**「忆灵在场时」：这是那 **8 条**已登记遗器 ability（123/124/127/318/319/320/321/323）
   共同的最后一块，做成一个条件变量即可一次覆盖；
3. **ATTACK / DEFENCE / 抗性**：没有任何文档给出这些数，本数据也没有忆灵面板表 → 保持 0，
   后果是"忆灵吃满伤害"（见 ROADMAP 的登记）；
4. **忆灵造成的伤害**：文档用的是「等同于忆灵 X% **生命上限**」或「等同于**召唤者**攻击力 Y%」，
   即"伤害按某个属性缩放"，与 §23.6 的"amount 是字面量"是同一个缺口；
