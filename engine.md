...
> ⚠ **"数值 + 具名 ability"混合的效果是登记表的盲区**（2026-09-27 实测）。
> `_unmodelled.json` 只覆盖 `properties` **为空**的纯 ability（35 条），而数据里还有
...
> `everyAbilityOnlyBonusIsEitherAuthoredOrRegistered` 想守住、但因为 `properties().isEmpty()`
> 这个过滤条件而没守到的地方。已登记为 **M-25**（ROADMAP §12.2），修法是把这个分母也纳入分区校验。
> 2026-09-27 已用新词表写掉其中 3 条（301 / 307 / 309），它们是**混合效果**，
> 所以计数**没有**变化 —— 这也说明分区测试的缺口是真实的，不是记账问题。
